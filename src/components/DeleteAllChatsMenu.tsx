import { useState, type MouseEvent } from "react";
import {
  Box,
  Button,
  Dialog,
  DialogContent,
  Divider,
  IconButton,
  Menu,
  MenuItem,
  Stack,
  Typography,
  useTheme,
} from "@mui/material";

import CloseIcon from "@diligentcorp/atlas-react-bundle/icons/Close";
import TrashIcon from "@diligentcorp/atlas-react-bundle/icons/Trash";

function VerticalDotsIcon() {
  return (
    <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path d="M9.99998 16.0577C9.65624 16.0577 9.36198 15.9353 9.11719 15.6905C8.8724 15.4457 8.75 15.1515 8.75 14.8077C8.75 14.464 8.8724 14.1697 9.11719 13.9249C9.36198 13.6801 9.65624 13.5577 9.99998 13.5577C10.3437 13.5577 10.638 13.6801 10.8828 13.9249C11.1276 14.1697 11.25 14.464 11.25 14.8077C11.25 15.1515 11.1276 15.4457 10.8828 15.6905C10.638 15.9353 10.3437 16.0577 9.99998 16.0577ZM9.99998 11.25C9.65624 11.25 9.36198 11.1276 9.11719 10.8828C8.8724 10.638 8.75 10.3438 8.75 10C8.75 9.65629 8.8724 9.36203 9.11719 9.11724C9.36198 8.87245 9.65624 8.75005 9.99998 8.75005C10.3437 8.75005 10.638 8.87245 10.8828 9.11724C11.1276 9.36203 11.25 9.65629 11.25 10C11.25 10.3438 11.1276 10.638 10.8828 10.8828C10.638 11.1276 10.3437 11.25 9.99998 11.25ZM9.99998 6.44232C9.65624 6.44232 9.36198 6.31992 9.11719 6.07513C8.8724 5.83036 8.75 5.53609 8.75 5.19234C8.75 4.84861 8.8724 4.55434 9.11719 4.30955C9.36198 4.06477 9.65624 3.94238 9.99998 3.94238C10.3437 3.94238 10.638 4.06477 10.8828 4.30955C11.1276 4.55434 11.25 4.84861 11.25 5.19234C11.25 5.53609 11.1276 5.83036 10.8828 6.07513C10.638 6.31992 10.3437 6.44232 9.99998 6.44232Z" fill="currentColor"/>
    </svg>
  );
}

interface DeleteAllChatsMenuProps {
  onDeleteAll?: () => void;
}

export default function DeleteAllChatsMenu({ onDeleteAll }: DeleteAllChatsMenuProps) {
  const { tokens: { semantic: { color } } } = useTheme();
  const [anchorEl, setAnchorEl] = useState<HTMLElement | null>(null);
  const [dialogOpen, setDialogOpen] = useState(false);
  const [deleting, setDeleting] = useState(false);
  const open = Boolean(anchorEl);

  const handleOpen = (e: MouseEvent<HTMLButtonElement>) => {
    e.stopPropagation();
    setAnchorEl(e.currentTarget);
  };
  const handleClose = () => setAnchorEl(null);

  const handleConfirmDelete = async () => {
    setDeleting(true);
    await new Promise((r) => setTimeout(r, 600));
    setDeleting(false);
    setDialogOpen(false);
    onDeleteAll?.();
  };

  return (
    <>
      <IconButton
        size="small"
        aria-label="More options"
        onClick={handleOpen}
        sx={{
          width: "24px",
          height: "24px",
          borderRadius: "6px",
          backgroundColor: open ? "rgba(0,0,0,0.06)" : "transparent",
          "&:hover": { backgroundColor: "rgba(0,0,0,0.06)" },
        }}
      >
        <VerticalDotsIcon />
      </IconButton>

      <Menu
        open={open}
        anchorEl={anchorEl}
        onClose={handleClose}
        anchorOrigin={{ vertical: "bottom", horizontal: "right" }}
        transformOrigin={{ vertical: "top", horizontal: "right" }}
        slotProps={{
          paper: {
            sx: {
              mt: "4px",
              borderRadius: "12px",
              boxShadow: "0 4px 16px rgba(0,0,0,0.12), 0 0 2px rgba(0,0,0,0.08)",
              minWidth: "200px",
              py: "8px",
              border: `1px solid ${color.ui.divider.default.value}`,
            },
          },
        }}
      >
        <MenuItem
          onClick={() => { handleClose(); setDialogOpen(true); }}
          sx={{ px: "16px", py: "12px", gap: 0, "&:hover": { backgroundColor: color.surface.variant.value } }}
        >
          <Stack direction="row" alignItems="center" gap="12px" sx={{ color: color.status.error.text.value }}>
            <TrashIcon size="md" />
            <Typography sx={{ fontSize: "14px", lineHeight: "20px", color: "inherit" }}>
              Delete chat history
            </Typography>
          </Stack>
        </MenuItem>
      </Menu>

      <Dialog
        open={dialogOpen}
        onClose={() => !deleting && setDialogOpen(false)}
        PaperProps={{
          sx: {
            borderRadius: "12px",
            width: "480px",
            maxWidth: "calc(100vw - 48px)",
            m: "24px",
            boxShadow: "0 8px 32px rgba(0,0,0,0.16)",
          },
        }}
      >
        <Box sx={{ px: "24px", pt: "20px", pb: "20px" }}>
          <Stack direction="row" alignItems="center" justifyContent="space-between">
            <Typography sx={{ fontSize: "18px", fontWeight: 600, lineHeight: "28px", color: color.type.default.value }}>
              Delete chat history
            </Typography>
            <IconButton
              onClick={() => setDialogOpen(false)}
              disabled={deleting}
              size="small"
              sx={{ ml: "8px", color: color.type.muted.value }}
            >
              <CloseIcon size="md" />
            </IconButton>
          </Stack>
        </Box>
        <Divider />
        <DialogContent sx={{ px: "24px", py: "20px" }}>
          <Typography sx={{ fontSize: "14px", lineHeight: "22px", color: color.type.default.value }}>
            All chats will be permanently removed, including questions and AI-generated responses. This cannot be undone.
          </Typography>
        </DialogContent>
        <Divider />
        <Box sx={{ px: "24px", py: "16px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <Button
            variant="outlined"
            onClick={() => setDialogOpen(false)}
            disabled={deleting}
            sx={{ borderRadius: "8px", textTransform: "none", fontWeight: 600, fontSize: "14px" }}
          >
            Cancel
          </Button>
          <Button
            variant="contained"
            onClick={handleConfirmDelete}
            disabled={deleting}
            sx={{
              borderRadius: "12px",
              textTransform: "none",
              fontWeight: 600,
              fontSize: "14px",
              background: `linear-gradient(180deg, ${color.action.destructive.defaultGradientStart.value} 0%, ${color.action.destructive.defaultGradientEnd.value} 100%) !important`,
              color: color.action.destructive.onDestructive.value,
              boxShadow: "none",
              "&:hover": { background: `linear-gradient(180deg, ${color.action.destructive.hoverGradientStart.value} 0%, ${color.action.destructive.default.value} 100%) !important`, boxShadow: "none" },
              "&:active": { background: `linear-gradient(180deg, ${color.action.destructive.activeGradientStart.value} 0%, ${color.action.destructive.activeGradientEnd.value} 100%)`, boxShadow: "none" },
              "&.Mui-disabled": { background: color.action.destructive.disabled.value, color: color.action.destructive.onDestructive.value, opacity: 1, boxShadow: "none" },
            }}
          >
            {deleting ? "Deleting…" : "Delete"}
          </Button>
        </Box>
      </Dialog>
    </>
  );
}
